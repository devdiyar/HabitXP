...
export default function RegisterScreen() {
  ...
  const handleRegister = async () => {
    ...
    if (!isNotEmpty(username))
      newErrors.username = 'Bitte Benutzernamen eingeben';
    if (!isNotEmpty(email)) newErrors.email = 'Bitte Email eingeben';
    if (!isValidEmail(email))
      newErrors.email = 'Bitte eine gültige Email-Adresse eingeben';
    if (!isNotEmpty(password)) newErrors.password = 'Bitte Passwort eingeben';
    if (!isSecurePassword(password))
      newErrors.password = 'Mind. 8 Zeichen und 1 Großbuchstabe';
    ...
    >
      <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
        <Container style={styles.container}>
          ...
          {errors.server && (
            <NormalText style={styles.serverError}>{errors.server}</NormalText>
          )}
